...
class WeightedEdge(Edge):
    def __init__(self, src, dest, total_distance, outdoor_distance):
        ...
        self.t_distance = int(total_distance)
        self.o_distance = int(outdoor_distance)
        
    def get_total_distance(self):
        return self.t_distance

    def get_outdoor_distance(self):
        return self.o_distance

    def __str__(self):
        return f"{self.src}->{self.dest} ({self.t_distance}, {self.o_distance})"
...
class Digraph(object):
    ...
    def add_edge(self, edge):
        ...
        in the  graph."""
        if not (self.has_node(edge.dest) and self.has_node(edge.src)):
            raise ValueError("Both end nodes of edge must be in graph")
        else:
            self.get_edges_for_node(edge.src).append(edge)
